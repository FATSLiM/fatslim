 ...
 * Partial coordinates decompression (c) by Sébastien Buchoux 2014.
 *
...
struct XDR
{
	...
	struct xdr_ops
	{
		...
		unsigned long (*x_getpostn) (XDR *__xdrs); 
		int (*x_setpostn) (XDR *__xdrs, unsigned long __pos);
...
int
xdrfile_decompress_coord_double_partial(double     *ptr, 
								int        *size,
								int		   *ncoords,
								double     *precision,
								XDRFILE*   xfp)
{
	...
	*size = lsize;
	...
	if(*size<=9)
    {
		tmp=xdrfile_read_float(tmpdata,*ncoords * 3,xfp);
		for(i=0;i< *ncoords *3;i++)
			ptr[i]=tmpdata[i];
		return tmp/3;
		/* return number of coords, not floats */
	}
...
static unsigned long xdrstdio_getpos (XDR *);
static int xdrstdio_setpos (XDR *, unsigned long);
...
{
	return (unsigned long) ftell ((FILE *) xdrs->x_private);
}

unsigned long xdrfile_getpos(XDRFILE* xfp)
{
    return xdrstdio_getpos (xfp->xdr);
}

static int
xdrstdio_setpos (XDR *xdrs, unsigned long pos)
{
	return fseek ((FILE *) xdrs->x_private, pos, 0) < 0 ? 0 : 1;
}

int xdrfile_setpos(XDRFILE* xfp, unsigned long pos)
{
    return xdrstdio_setpos (xfp->xdr, pos);
}
